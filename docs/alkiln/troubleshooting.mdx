...
To troubleshoot syntax typos, you can use an editor like the [editor at AssertThat](https://www.assertthat.com/online-gherkin-editor). It will let you paste in your test code and will show a red 'x' next to the lines that have syntax errors.
...
This error prevents all of your tests being run. The message is telling you that something about the syntax of the table is wrong. One of your story tables could missing a pipe (`|`) or could have an extra pipe, etc. This is an error from a library ALKiln uses.
...
This is an error message from a library ALKiln uses called cucumber.
...
<!-- writing_tests.mdx#error-pics, writing_tests.mdx#reports -->


## UnhandledPromiseRejection error

This is a misleading error from a library ALKiln uses that is hiding the real error message inside it. You need to read the text of the whole paragraph to see what the actual error is.
...
2. The field is hidden behind a `show if` of some kind.
...
3. There might be a typo in your test. Reading the [section about test typos](#var-val) might help.


## Docassemble: Variable could not be looked up {#variable-not-found}

<!-- TODO: Actually, usually shows up as ALKiln couldn't find a field -->
...
If you look in a [test suite report](writing_tests.mdx#report) and see that ALKiln [skipped setting a variable](writing_tests.mdx#unused-rows), your test might have a [missing field or value](#missing).
...
Make sure every question block you create has an id (link to FAQ)
-->
...
If ALKiln is missing all [artifacts](writing_tests.mdx#artifacts) by the end of the test run, it means none of the tests ran. There are possible several reasons for this.
...
- The Docker container is having trouble starting up.  If this was the case, your [GitHub job logs](writing_tests.mdx#console) should show that the docker build step failed.
...
- Dependency package missing on server ("contact the server administrator")
    ...
    For your own server, make sure you've installed the right package
- File missing in package? (not sure if it's the same error)
...
TODO: note when tests properly starting
-->

<!--
Behaves differently in Sandbox than in G+Y and AutoD
...
<!-- This is different than a never-ending test/infinite loop where there was no validation error, but the test can't move past the page. -->

**This is for:** Everyone

There are a few different reasons your tests could be slow.

1. Failing tests can make your tests slower. Each failing test gets re-run once, so that can double the amount of time that tests take.
2. If your server reloads a lot during <GTOYS/> tests, it will take longer for tests to finish because the tests try to wait for the server to be available to avoid ["flaky"](#flake) tests. Your server can reload for many reasons. For example, saving your config file, saving a python module, or pulling a package that uses a python module can all cause your server to reload.
3. Some types of failing tests take an even longer time to fail. That is because some kinds of failures look the same to ALKiln as a docassemble server reloading. This is true even for <KittyLitter/> tests where the server is very unlikely to reload - ALKiln still treats those failures the same way.
...
<KittyLitter/> test have the best chance of avoiding flakiness because they get their own dedicated server, but even they can flake out sometimes.
...
Trouble with server reloading for <GTOYS/> tests

Option 1:
`SERVER_RELOAD_TIMEOUT_SECONDS` customizes how long to allow your tests to wait when your server has stopped responding. When the server stops responding, ALKiln assumes it is because the server is reloading. A server reload stops a test part-way through and and causes a failure. You then are unable to tell whether the test would have passed or failed on its own. Because servers usually take a while to reload, this can cause multiple tests to fail. If you know your server tends to be slow to reload, you can increase `SERVER_RELOAD_TIMEOUT_SECONDS` to help tests recover better.

The server can reload for many reasons. For example, when someone saves the server config file. In fact, in some cases [a different test run starting up can cause a reload](setup.mdx#githubNyou-cons) too.

Unfortunately, there are some real errors that make ALKiln think the server might be reloading. Those tests will also slow down when they fail. That can mean failing tests can take a lot longer. There is currently no way for ALKiln to tell the difference.

Option 2:
The SERVER_RELOAD_TIMEOUT_SECONDS environment variable allows developers to specify the wait time for ALKiln tests when the server stops responding, typically due to a reload. This setting helps manage test failures caused by server reloads, which can occur for various reasons, such as configuration changes or new test runs. However, it also affects tests that fail due to genuine errors, leading to longer test durations. Currently, ALKiln cannot distinguish between these scenarios.

More:
ALKiln waits till it detects that the server has reloaded. It can't wait forever or a broken server will cause infinite tests. So it picks a maximum time to wait.

That first test will fail for unhelpful reasons. ALKiln will wait a while to try to let the server reload. That first failing test will be retried. If the server has finished reloading, the test will be able to finish properly. Even if that isn't long enough, maybe the next tests will be able to run properly once the reload is finished.
-->
