...
This page will try to help you figure out what is going wrong when you have errors, warnings, or other problems with your ALKiln tests. This document is still under development and contains only some of the possible errors.
...
<!-- Every ALKiln error and warning should have a code, though, and giving us that code can help us narrow down your problem. -->
...
## Debugging syntax errors {#gherkin}

WIP

TODO: Link to Gherkin section.
...
Example syntax typo errors:

- [Inconsistent cell count](#table-cells)


## Everything works fine when I run the test manually {#manual-works}

Or "It works when I run the interview manually" or "The tests pass when I run them with <AutoDIY/> tests".
...
This is a misleading error from a third-party library that is hiding our error message inside it. You need to read the text of the whole paragraph to see what the actual error is.
...
- ALKiln's third-party libraries were unavailable
- Your server reloaded

For GitHub tests:

<!-- TODO: Link to "developer account" account for testing description -->
- Your docassemble developer account credentials were invalid. Maybe they got deleted in an update. Maybe someone else changed them. You need a new API key for that developer account.
- Your test server was unavailable
- GitHub itself had a problem
...
One reason for slow <GTOYS/> tests are server reloads.

This is different than infinite tests.
