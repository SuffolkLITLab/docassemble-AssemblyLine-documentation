...
<!--
- People managing GitHub organizations can simplify future ALKiln test setup for their developers with secrets
...
1. Follow [docassemble's instructions](https://docassemble.org/docs/config.html#dispatch) to add it to the dispatch list. This will add it to the server's list of interviews. It may look something like this:
  ...
  ```
1. Go to your server's list of ["Available interviews"](https://docassemble.org/docs/admin.html#available%20interviews). ALKiln's package should now be listed there.
...
<GTOYS/> tests are [triggered when you commit code to GitHub](writing_tests.mdx#trigger-tests). They then create and test a Project on your docassemble [testing server](security.mdx#testing-server). They are our recommended test method because they are the easiest to set up.
...
Because <GTOYS/> tests need to create a Project on your [testing server](security.mdx#testing-server) and pull code into it, you need to create a [developer account](security.mdx#alkiln-account) for these types of tests.
