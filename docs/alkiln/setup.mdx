...
<!-- Doc about: things managers/admins do, testing modalities. Other docs shouldn't have to deal with differences between GitHub1, GitHub2, Playground. -->
...
<!-- TODO: Should this include how to run the tests? Workflow files, trigger automatically, on a schedule, manually, tags, ALKiln versions, etc. Pros and cons are already kind of about running the tests. This is separate from providing all the necessary files. -->
<!-- Workflow section about an action version vs. an alkiln version? -->
...
The setup interview will try to walk you through this.
...
Isolated server tests are the most secure because they never interact with your server. They run on GitHub too, we just have not yet found better names for these test types. ALKiln makes a server on GitHub itself and runs the tests there.
...
<!-- Read more about GitHub test options here. -->
...
- You can avoid putting your package on GitHub to run the tests.
...
- You will miss catching some kinds of mistakes. ["File not found" errors](trouble#file-not-found) can come up because of this.
