...
1. GitHub tests that create and test a Project on your docassemble server
2. Playground tests that you run manually from an interview you install on your server
3. Isolated server tests that create their own docassemble server on GitHub and run tests there
...
Isolated server tests are the most secure because they never interact with your server and they are temporary. Unfortunately, they also sometimes have more setup complications.

These tests actually run on GitHub too, we just have not yet found better names for the different test types. You can read about [running tests](writing_tests.mdx#github-triggers) in other documentation.
