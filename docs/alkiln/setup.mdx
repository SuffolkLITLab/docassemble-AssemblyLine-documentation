...
Maybe not. People just writing tests can skip this page. Also, if your package's repository already has a [`.github` folder with a workflow file](writing_tests.mdx#workflows-where) you can probably skip this page.
...
The ALKilnInThePlayground package installs the ALKiln testing framework directly on your server so you can run <AutoDIY/> tests. For [security reasons](security.mdx#testing-server), avoid installing these tests on your production server.
...
You can also [delete](https://docs.github.com/en/repositories/working-with-files/managing-files/deleting-files-in-a-repository) the [workflow file](writing_tests.mdx#workflows-where) from your repository completely.
