...
1. If your interview is using generic objects or index variables, you **must** include special HTML <!-- TODO: Add link to setting special variables. Why is it that `trigger` is still needed? There is a reason, but I don't remember what it is --> in your interview.
...
The first set of quotes contain the name of the variable you need to set. The second set of quotes contain the value you want to set. This is the same as the [Story Table's](#story-tables)'s `var` column and `value` column.
...
These are fields that a user needs to type in. The [default field type](https://docassemble.org/docs/fields.html#plaintext) is a text field. Other text fields include those with:
