...
1. If you are using <GTOYS/> or <KittyLitter/> tests, those tests will run when anyone commits to GitHub.
...
1. ALKiln creates test reports and pictures. For example, ALKiln saves pictures and the HTML of pages that showed errors. In GitHub, those files are in the zip file in [your workflow's artifact section](https://docs.github.com/en/actions/managing-workflow-runs/downloading-workflow-artifacts).
...
The words of this Step currently assume you use GitHub for your tests and are using a GitHub secret to store the value, but this Step also works with config values you create for <KittyLitter/> tests. You can [read more about environment variables here](#env-vars).
...
**This is for:** any packages that run GitHub tests. That is, <GTOYS/> or <KittyLitter/> tests.

<!-- use [the <GTOYS/> test type](setup.mdx#recommended). -->
<!-- use [the <KittyLitter/> test setup](setup.mdx#sandbox-how-to). -->

**It can help to know about:**

<!-- https://docs.github.com/en/actions/using-workflows/about-workflows -->
<!-- seeing workflow job logs: https://docs.github.com/en/actions/monitoring-and-troubleshooting-workflows/using-workflow-run-logs -->

- [GitHub workflow files](https://docs.github.com/en/actions/using-workflows)
...
You can see [ALKiln's own version of the <GTOYS plain={true}/> test workflow file here](https://github.com/SuffolkLITLab/ALKiln/blob/v5/.github/workflows/playground.yml).

You can see [ALKiln's own version of the <KittyLitter plain={true}/> test workflow file here](https://github.com/SuffolkLITLab/ALKiln/blob/v5/.github/workflows/github_server.yml).
...
### Required inputs for <GTOYS/> tests {#auto-server-inputs}
...
- `DOCASSEMBLE_DEVELOPER_API_KEY` is a developer API key for a developer on your server. When you set up for this kind of test, we recommend you create a new developer account on your server just for testing. This developer account will [store the Project in its Playground while GitHub runs the tests](setup.mdx#github-how).
...
### Required inputs for <KittyLitter/> tests {#sandbox-inputs}

<!-- TODO: Add link to Sandbox test info -->

These are the only required inputs for <KittyLitter/> tests. We are listing them here for the sake of completeness, but none of them need to be secrets and you can copy the correct values straight from [ALKiln's own <KittyLitter plain={true}/> tests workflow file](https://github.com/SuffolkLITLab/ALKiln/blob/v5/.github/workflows/github_server.yml).
...
- `INSTALL_METHOD` is an internal value ALKiln needs from <KittyLitter/> tests. The value in ALKiln's file is the correct value.
...
None of these need to be secrets, but they can be if you want that[^org]. You will probably want to avoid using secrets for `ALKILN_VERSION` and `ALKILN_TAG_EXPRESSION`. Authors usually use them for a specific occasion for a specific reason.
...
### Optional inputs for <GTOYS/> tests {#github-optional-inputs}
...
### Optional inputs for <KittyLitter/> tests {#sandbox-optional-inputs}
...
Your [workflow file](#workflows) controls when your GitHub tests run. You can [configure your workflow file to run the tests whenever you want them to run](https://docs.github.com/en/actions/using-workflows/triggering-a-workflow).
...
<!-- TODO: Add note about tags in FAQ? Or Tips? Or here? Or workflow files? -->

### When do <AutoDIY/> tests run? {#diy-triggers}

You run them manually when you want to.
...
<!-- TODO:
Where to put how to update to a new version of ALKiln? It's already in setup, but buried.
-->


### What can I see while tests are running? {#running-output}

<!-- TODO: Link to workflow status on Actions page -->
In GitHub, while the tests are running, GitHub can show you the status of the workflow/test run on the Actions page or at the bottom of a PR, among other places - you can see a yellow dot while tests are still running and then, when the test run is done, a green check mark for successful tests or red "x" when something went wrong. You can see how long a test has been running. You can also see the current [console log](#console) of the workflow job as GitHub writes in it.

Unfortunately, <AutoDIY/> tests just show how long the tests have been running.
...
- Console logs: The raw output of the whole test run process in the [console](#console)
- Artifacts: The reports and other files ALKiln creates to give you more information about the tests, including errors

You can read more about [errors and troubleshooting in other documentation](troubleshooting.mdx).


## Test console logs {#console}

The console shows the full raw output of the logs of the processes that ran before, during, and after the tests.

In GitHub, you can see the test run's console at the [workflow's job logs](https://docs.github.com/en/actions/monitoring-and-troubleshooting-workflows/using-workflow-run-logs). You can see these messages appear as each process happens as well as after the tests finish running.

In <AutoDIY/> tests, you can only see the console output after tests are done. They should be at the bottom of the final page.

The console output should include, from top to bottom:

1. Data from the start up processes, where errors can happen too.
2. Each Scenario (test) description next to symbols that show what happened in that test:

- `.` is for a Step that just got run or the automatic start or end of a test
- `*` is for a Story table row that set a page variable
- `v` is for a Story table row that pressed a continue button to set a variable
- `>` is for a Story table row that automatically continued
- `U` usually shows up when you have a [typo in a Step you wrote](troubleshooting.mdx#step-undefined)
- `F` appears if a test fails and stops
...
1. In GitHub, data from the takedown processes as ALKiln and GitHub cleans up.
...
**This is for**: any tests.

Once the tests are finished, ALKiln creates the test run "artifacts" - folders and files you can examine, like an archeologist, to understand what happened.

<!-- TODO: insert archeology meme -->
...
For <AutoDIY/> tests, you can see and download the artifacts on the final page. The page is very plain at the moment. We hope to add more useful styles and indicators in the future. The page includes:
...
- A list of Scenarios with the [contents of their Scenario folders](#senario-artifacts).
- The test console logs. <!-- Why can't people download this as a file? I guess it will be in the verbose log eventually? -->
...
The main artifact folder contains all the other artifacts. Its name includes the time the tests started in UTC format.

In GitHub, GitHub wraps folder up in a zip file. The zip file's name includes the UTC time that the GitHub run started.

The ALKiln folder contains this structure of files and folders:

- [report.txt](#report)
- [debug_log.txt](#debug-log)
- Copies of all [screenshots and HTML files](#errors) of pages all that caused errors
- Scenario folders:
...
This is the first place to look for what went wrong.
...
### Scenario artifacts {#senario-artifacts}

### Scenario folder {#scenario-dir}
...
- Directly in the main folder. This is so you can have a quick look without digging through all the Scenario folders. There the file names use the Scenario description and the id of the page where the error happened.
- In the [Scenario folder](#senario-artifacts). There the file names use just the id of the page where the error happened.
