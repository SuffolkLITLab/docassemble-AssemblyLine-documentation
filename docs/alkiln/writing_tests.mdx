...
All tests that fail will get run a second time. These retries can make tests take longer, but they can also help with [flaky failing tests](troubleshooting.mdx#flake) caused by [server reloads](troubleshooting.mdx#slow).
