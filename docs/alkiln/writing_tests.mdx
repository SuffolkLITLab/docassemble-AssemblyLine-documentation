...
The Story Table Step fills in fields on interview pages. It is the Step many of our authors work with the most. In our opinion, it is the most flexible way to fill in fields in your forms. This makes a test easier to maintain.
...
As you can see, there are three types of blanks you need to fill in for this Step: `<target question id>`, `<variable name>`, `<value to set>`.
...
[^id] You can find the question `id` in your interview code in the `question` block of that page. It is a good idea for every `question` block in your interview to have an id.
