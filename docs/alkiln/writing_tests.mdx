...
1. ALKiln creates test reports and pictures. For example, ALKiln saves pictures and the HTML of pages that showed errors. In GitHub, those files are in the zip file in [your workflow's artifact section](https://docs.github.com/en/actions/managing-workflow-runs/downloading-workflow-artifacts). bookmark
...
**This is for:** all test that run on GitHub. That is, <GTOYS/> or <KittyLitter/> tests.
 run
...
seeing workflow job logs: https://docs.github.com/en/actions/quickstart#viewing-your-workflow-results
-->

<!-- Is this the workflow bookmark we want to use here? Probably, for general workflow information -->
...
### `MAX_SECONDS_FOR_SERVER_RELOAD` environment variable {#temp-server-reload}

**This is for:** <GTOYS/> tests

<!-- TODO: Add link to env vars -->

`MAX_SECONDS_FOR_SERVER_RELOAD` is an environment variable that ALKiln has a specific use for. This is a mistake and we plan to make it an input in the future.

<!-- Unlike the [inputs](#workflow-inputs), it goes under the `env:` section in your workflow file. -->
...
`MAX_SECONDS_FOR_SERVER_RELOAD` customizes how long to allow your tests to wait when your server has stopped responding. When the server stops responding, ALKiln assumes it is because the server is reloading. A server reload stops a test part-way through and and causes a failure. You then are unable to tell whether the test would have passed or failed on its own.

The server can reload for many reasons. For example, when someone saves the server config file. In fact, in some cases [when a different test run starts up, it can cause your server to reload](setup.mdx#githubNyou-cons). Increasing `MAX_SECONDS_FOR_SERVER_RELOAD` can help tests recover better, giving an affected test a second chance to run properly.

<!-- You can read the environment variables section on setting arbitrary environment variables for more background information. -->
...
If you want to dig deeper into this, [GitHub's documentation can tell you much more about workflow files](https://docs.github.com/en/actions/using-workflows).
...
<!-- Best instructions for seeing workflow, action, and jobs: https://docs.github.com/en/actions/quickstart#viewing-your-workflow-results bookmark -->

You can also [run your tests manually](https://docs.github.com/en/actions/using-workflows/manually-running-a-workflow#running-a-workflow) whenever you want. When you run your tests manually, you can use [tags](#tags) to limit which tests to run.

You can also configure your [workflow file](#workflows) to run your tests whenever you want. [GitHub's documentation for workflow triggers](https://docs.github.com/en/actions/using-workflows/triggering-a-workflow) is more thorough than our documentation.
...
All tests that fail will get run a second time. This can make tests take longer, but it can also help tests be more accurate.
