...
Environment variables are one of the few ways for you to get information from GitHub or your server to ALKiln. <!-- TODO: Link to inputs? -->

In <AutoDIY/> tests on your server, you put these values in your config file under a key you can add called `alkiln`. For example:
...
In GitHub, you can make environment variables by putting them in the `env` key in your [workflow file](#workflows):
...
All tests that fail will get run a second time. These retries can make tests take longer, but it can also help with flaky failing tests caused by [server reloads](troubleshooting.mdx#slow).
