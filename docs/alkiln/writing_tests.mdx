...
Each of your tests - [a `Scenario` or a `Scenario Outline`](#test-files) - usually contains many Steps. Any Step can be used as many times as you want anywhere in a test. If you follow [good code-writing practices](#choice-values), most Steps work in any language.
...
You can also instead use the full url of the interview, though we recommend you avoid that. For example, one advantage to using the file name instead of the full url is that you can then use the [isolated server tests method](setup.mdx#isolated-server-details) to run your tests on GitHub.
...
Use the "sign in" Step to sign into your docassemble server before going to the interview with the ["start the interview" Step](#start-interview):

```gherkin
  Given I sign in with the email "USER_EMAIL" and the password "USER_PASSWORD"
```

You **must** use [environment variables](#env-vars) for the values in this command to keep the user log in information secure. `"USER_EMAIL"` and `"USER_PASSWORD"` are just examples of environment variable names. You can use any names you want.

ALKiln will avoid taking a screenshot of this page or saving the HTML of the page. It also will avoid logging or printing these values. It is a bad security practice to take pictures of or print sensitive information like the information on login pages.
...
AssemblyLine's al_package.yml file, if you include it, already adds that HTML for you. Otherwise, we strongly recommend adding it yourself as the docs described.
...
Unlike many other Steps, checking phrases is **language specific**. That is, if you want to test translations as well, you will have to use [extra code](#gherkin-examples) in your test.
...
The "picture" Step will not only take a picture of the page, it will also download an HTML file of the page. ALKiln will save those files in the Scenario folder of the [test results](#results).

The HTML file will look and act a bit strange because ALKiln is unable to get the full code that controls and styles the page, but you can look at it and examine its source code.
...
### `I get the page's JSON variables and values` {#show-vars}
...
<!-- TODO: Does this save a file with the vars as well? -->
...
You can then add the code for your tests. Follow this link to see a [simple test](setup.mdx/#first-test) that any interview can run right from the start.


### Some rules {#gherkin-rules}
...
- `@` ([Tags](https://cucumber.io/docs/cucumber/api/?lang=java#tags)). You can use tags to limit which tests to run. The specific sections of the tags documentation that apply to ALKiln are:
...
You can still test translations even when you use a language-specific Step, like the ["phrase" Step](#phrase). [Check Gherkin examples](#gherkin-examples)
...
**This is for:** packages that run tests in GitHub.

<!-- use [the recommended GitHub test type](setup.mdx#recommended-setup). -->
<!-- use [the isolated server test setup](setup.mdx#isolated-server-how-to). -->
...
This section has a lot of challenging technical information about [GitHub workflow files](https://docs.github.com/en/actions/using-workflows). We will do our best to help. We would love to hear how we can improve.


### What is this file? {#workflows-what}

Your GitHub workflow file is what starts and controls a package's tests on GitHub. Only [tests that run on GitHub](setup.mdx#types) need a workflow file. The file gives ALKiln some configuration information. You can use it to customize the ALKiln test runs and control when to trigger the GitHub tests.

<!-- You can do a lot more with your workflow file if you study details about [GitHub workflows](https://docs.github.com/en/actions/using-workflows) and the [GitHub CLI](https://docs.github.com/en/actions/using-workflows/using-github-cli-in-workflows), like making issues when tests fail. ALKiln uses only a few features. -->
...
### Can I see an example file? {#workflows-examples}

ALKiln's own workflow files are the most up-to-date, so using those as an example can be useful. You cannot just copy them. They have features that are unique to ALKiln. They do have notes for how to change them to what you need.

You can see [ALKiln's own version of the GitHub test workflow file here](https://github.com/SuffolkLITLab/ALKiln/blob/v5/.github/workflows/playground.yml).

You can see [ALKiln's own version of the isolated server test workflow file here](https://github.com/SuffolkLITLab/ALKiln/blob/v5/.github/workflows/github_server.yml).
...
### What are the values I can customize in the file? {#workflows-values}

You can use [workflow inputs](#workflow-inputs) to change global configuration settings for all your whole test run.

You can use custom [environment variables](#env-vars) to set values in your tests.


### Security {#workflows-security}

See the [security documentation](security.mdx).


## Workflow inputs {#workflow-inputs}

Workflow input values let you customize global ALKiln configuration settings. This section is advanced and can be challenging for even experienced developers. Let us know how we can improve it.


### Required inputs for recommended GitHub tests {#auto-server-inputs}

The [recommended GitHub test method](setup.mdx#recommended-setup) workflow file has a few absolutely **required** inputs. The [ALKiln setup interview](https://apps-dev.suffolklitlab.org/start/test-setup) should create the secrets and workflow inputs for this type of test environment.
...
- `DOCASSEMBLE_DEVELOPER_API_KEY` is a developer API key for a developer on your server. When you set up for this kind of test, we recommend you create a new developer account on your server just for testing. This account will store [the Project in the Playground while GitHub runs the tests](setup.mdx#github-how).

If you do not already have these secrets, add them to your package's repository or GitHub organization[^secret].
...
[^secret]: If your server is part of a GitHub organization, we recommend creating some of these values as organization secrets. That way, if you need to change the value of an input or environment variable you can do it one place.


### Required inputs for isolated server tests {#isolated-server-inputs}

<!-- TODO: Add link to isolated server test info -->

These are the only required inputs for isolated server tests. We are listing them here for the sake of completeness, but none of them need to be secrets and you can copy the correct values straight from [ALKiln's own isolated tests workflow file](https://github.com/SuffolkLITLab/ALKiln/blob/v5/.github/workflows/github_server.yml).

You can see examples of these inputs in the correct [ALKiln file](#workflows-examples).

- `SERVER_URL` is the url of the new docassemble server GitHub just created. Another step in the same file defined that value.
- `DOCASSEMBLE_DEVELOPER_API_KEY` is a developer API key for a developer on the new server. Another step in the same file defined that value.
- `INSTALL_METHOD` is an internal value ALKiln needs from isolated server tests. The value in ALKiln's file is the correct value.


### Optional inputs for all tests that run on GitHub {#optional-inputs}
...
- `ALKILN_VERSION` lets you use a specific version of the ALKiln framework for your tests. It can be useful for [security](security.mdx#freeze-framework) or for trying out experimental versions of the ALKiln framework. The default value is the latest version.
...
None of these need to be secrets, but they can be if you want that[^secret]. You will probably want to avoid using secrets for `ALKILN_VERSION` and `ALKILN_TAG_EXPRESSION`. Authors usually use them for a specific occasion for a specific reason.
...
### Optional inputs for GitHub test inputs {#github-optional-inputs}

WIP

You can add some optional inputs to the workflow file of any tests that run with the [recommended GitHub test method](setup.mdx#recommended-setup) to control global ALKiln configuration values:
...
- `SERVER_RELOAD_TIMEOUT_SECONDS` is the maximum amount of time to give the server to reload if it needs to. A server reload can make a test fail. Waiting the right amount of time for the reload can make it more likely the test will succeed when ALKiln retries it. The default is 150 seconds.

Use these inputs in the same way and in the same places as the required inputs. You can use non-secret values by just writing the value directly in the workflow file. For example:

```yml
        with:
          MAX_SECONDS_FOR_SETUP: 500
```

### Optional inputs for isolated server inputs {#isolated-server-optional-inputs}

You can add some optional inputs to the workflow file of any tests that run with the [isolated server method](setup.mdx#isolated-server-details) to control global ALKiln configuration values. We note when an option should also be a GitHub secret for security reasons:
...
- `SHOW_DOCKER_OUTPUT` you can give this the values "true" or "false". When you set it to "true", you will see more output from the creation of the docker container and the installation of docassemble there. The output will be visible to anyone that can see the action running, like GitHub admins or collaborators. By default, it is "false". This is because our team is unsure about what information it might include, so we are unsure if it will show sensitive configuration information, like tokens and API keys.
...
### When do GitHub tests run? {#github-triggers}

**This is for:** tests that run on GitHub.

Your [workflow file](#workflows) controls when your GitHub tests run. You can configure your workflow file to run the tests whenever you want them to run.

The default workflow file the ALKiln setup interview gives you triggers GitHub tests when you commit your files to GitHub. That might be when you hit the 'Commit' button on the docassemble Packages page. It can also happen when you edit, add, or delete files in GitHub itself.

That workflow file also lets you [run your tests manually](https://docs.github.com/en/actions/using-workflows/manually-running-a-workflow#running-a-workflow) whenever you want. There, you can use tags to limit which tests to run. You can read more about giving tags for tests in the section about [test files](#test-files).

<!-- TODO: Add note about tags in FAQ? Or Tips? -->
...
**This is for:** packages that use GitHub tests.
...
There are different ways to look at the results of a test run. GitHub shows a log of the tests while they run and ALKiln creates files and folders showing the output of the finished tests.

### GitHub outputs {#github-results}

In GitHub, you can see the workflow/test run status...

 test run's [workflow job logs](https://docs.github.com/en/actions/monitoring-and-troubleshooting-workflows/using-workflow-run-logs#viewing-logs-to-diagnose-failures) while the tests are still running or after they're done. Those logs show:

- Test run start up messages. Errors can happen here too.
- Scenario names as each test runs.
- The final ALKiln report.
- If there are any, errors from [the third-party libraries ALKiln uses](#dependencies). We are keeping the errors from the third-party libraries until we handle all cases of the errors ourselves.

Once the tests are finished, ALKiln also creates more detailed files for you. You can [download these GitHub "artifacts"](https://docs.github.com/en/actions/managing-workflow-runs/downloading-workflow-artifacts) at the bottom of the summary page for that workflow run.


- GitHub test status
- Console in GitHub
- GitHub artifacts (folders)
- ALKilnInThePlayground final page (and folders to download)
  - Not much fancy formatting yet
  - Zip file at top
  - Summary files up at top
  - List of Scenarios with report and other files
  - Console output at bottom

You can also read more about [errors and troubleshooting in other documentation](troubleshooting.mdx).


### GitHub test status {#github-status}
...
The console has all the text that got printed while your test ran. It has things like logs from starting up the tests, a copy of the report, and at the bottom are messages from the third-party libraries ALKiln uses. We are keeping those messages, including any errors, until we are confident we can handle every error ourselves.

In GitHub, you can see the console when you go to the ALKiln workflow job. In ALKilnInThePlayground, it's at the bottom of the page.

<!-- In GitHub, if the tests are having trouble, you might want to look at early steps too (TODO: Put this in troubleshooting docs) -->


### Output directory structure {#output-dirs}
...
- root
  - report.txt
  - debug_log.txt
  - Screenshots and html files for errors to give a quick way to look at them
  - Scenario folders
    - Individual report.txt
    - Error screenshots and html files
    - [Pictures you took of screens along with their associated HTML files](#pic) (self explanatory?)
    - [Files you downloaded](#download) (self explanatory?)
    - [JSON variables you compared](#compare-json) (self explanatory?)
    - [Accessibility failures](#a11y-all) (self explanatory?)
...
- At root: Summary report containing all Scenario reports
- In Scenario folders: Individual Scenario reports
...
Collects report data during the test run in case something goes wrong and the report for a test gets left out.

<!-- ### verbose_log.txt {#verbose-log} -->


### Error pics and html {#errors}

WIP

Also see the [troubleshooting docs](troubleshooting.mdx).

- In the root
- In the individual Scenario folders

They are like the [pictures you take of screens along with their associated HTML files](#pic).
...
Add a [different id](https://docassemble.org/docs/modifiers.html#id) to each `question` block of your interview. They helps make test output more clear. They also help your team communicate with each other about your code more easily.


### Write unique Scenario descriptions {#scenario-descriptions}

ALKiln uses Scenario descriptions in your [test files](#test-files) to make the names of test folders, error screenshot files, and report headings, so try to write something different for each one - something that you will recognize later. It will help you identify what happened with each test.


### Extra HTML {#html-tip}

If your package is leaving out the al_package.yml file from the Assembly Line package, make sure to add some [special ALKiln-specific HTML](#special-html) to your interview.


### Testing unfinished code {#unstable}
...
<!-- The value can even be the same as the label:
...
As a bonus, this also makes your interview easier to translate.
...
### I have a private GitHub repository. Can I use this testing framework? {#private-repo}
...
No, only the variables that users are required to set.
