...
The "picture" Step will take a picture of the page and download the HTML of the page. ALKiln will save those files in the Scenario folder of the [test results](#results).
...
## Understanding test results {#results}
...
That workflow file also lets you [run your tests manually](https://docs.github.com/en/actions/using-workflows/manually-running-a-workflow#running-a-workflow) whenever you want. There, you can use tags to limit which tests to run. You can read more about giving tags to your tests in the section about [test files](#test-files).
...
Use them in the same way and in the same places as the required inputs. You can use non-secret values by just writing the value directly in the workflow file. For example:
...
- `MAX_SECONDS_FOR_SETUP` is the maximum amount of time to give the Project to upload your package's GitHub code. If your server is slow, increase this value.
- `SERVER_RELOAD_TIMEOUT_SECONDS` is the maximum amount of time to give the server to reload if it needs to. A server reload can make a test fail. Waiting the right amount of time for the reload can make it more likely the test will succeed when ALKiln retries it.
...
They look similar to us, but computers think they look quite different.
