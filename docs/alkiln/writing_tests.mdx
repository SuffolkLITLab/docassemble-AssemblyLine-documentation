...
You **must** use [environment variables](#env-var) for the values in this command to keep the user log in information secure. `"USER_EMAIL"` and `"USER_PASSWORD"` are just examples of environment variable names. You can use any names you want.
...
That workflow file also lets you [run your tests manually](https://docs.github.com/en/actions/using-workflows/manually-running-a-workflow#running-a-workflow) whenever you want. There, you can use tags to pick specific tests to run. You can read more about giving tags to your tests in the section about [test files](#test-files).
...
## Your GitHub workflow file {#workflows}

WIP

**This is for:** packages that use GitHub tests.

Your ALKiln workflow file is in your GitHub repository. To find it, go to your `.github` folder, then open the `workflows` folder there. It was probably created when you ran the setup interview and it might be called "run_form_tests.yml" or "alkiln_tests.yml" or something similar.
