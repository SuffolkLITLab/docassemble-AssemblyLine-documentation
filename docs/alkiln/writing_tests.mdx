...
That said, your [workflow file](#workflows) controls when your GitHub tests run. You can [configure your workflow file to run the tests whenever you want them to run](https://docs.github.com/en/actions/using-workflows/triggering-a-workflow).
...
<!-- Note to self: BEST instructions for getting to github workflows and jobs: https://docs.github.com/en/actions/quickstart#viewing-your-workflow-results -->

In GitHub, on [the actions page and related pages](https://docs.github.com/en/actions/quickstart#viewing-your-workflow-results) of your repository, you can see a few different things while the tests are running:

- On the GitHub actions page, you can also see how long a test has been running.
...
The console, or GitHub job log, shows the full raw output of the logs of the processes that ran before, during, and after the tests.
...
Once the tests are finished, ALKiln creates the test run "artifacts" - files and folders you can examine, like an archeologist, to understand what happened.
...
<p><img src={Dig} className={`${styles['w-75p']} ${styles.center}`} alt="When the archaeologists opened the ancient vase, little did they know what primeval dark power they had unleashed. A cat in a vase."/></p>

\- [@Thinkwert](https://twitter.com/Thinkwert/status/1428488345357856777)[^aha]
...
- The test console logs. <!-- TODO: Why can't people download this as a file? I guess it will be in the verbose log eventually? -->
...
The main artifact folder contains all the other artifacts. Its name includes the time the tests started in UTC format. When in GitHub, GitHub wraps folder up in a zip file. The zip file's name includes the UTC time that the GitHub run started.
...
[^aha]: Name unknown [@Thinkwert]. (2021, Aug 19). *When the archaeologists opened the ancient vase, little did they know what primeval dark power they had unleashed.* [Tweet]. Twitter. https://twitter.com/Thinkwert/status/1428488345357856777
