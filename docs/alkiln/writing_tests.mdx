...
<!-- TODO: The tests use the [gherkin language and syntax](https://cucumber.io/docs/gherkin/reference/) -->
...
Use the "sign in" Step to sign into your docassemble server before going to the interview with the ["start interview" Step](#start-interview).
...
:::danger Use GitHub secrets
You **must** use GitHub secrets to set the [environment variables](#env-vars) in order to keep the information secure.
:::

The `<username env var>` variable should contain the username of an account on your docassemble server. The `<password env var>` variable should contain the password of that account. [Never use a real person's account or information](security.mdx#mock-data).
...
ALKiln avoids taking pictures or downloading the HTML for this Step, even when the test has an error on that page. ALKiln avoids printing the value of these variables anywhere in the report or in the console.
...
:::info
The words of this Step currently assume you use GitHub for your tests and are using a GitHub secret to store the value, but this Step also works with config values you create for <KittyLitter/> tests. They are all [environment variables](#env-vars).
:::
...
<!-- TODO: move to a section of #env-vars maybe -->

You can use this Step for more than just sensitive information, though. Suppose Mo'nique has a GitHub organization named PolarisLaw. That organization has several repositories. Every test in every repository uses the value "Small bear" as an answer to a question. Next month, though, the firm may need to change that value to "Large bear". Mo'nique would have to edit that value in every test in every repository. Instead, she makes an environment variable in the PolarisLaw account called `ALKILN_BEAR_SIZE`:

```yml
env:
  ALKILN_BEAR_SIZE: "Small bear"
```
...
Next month, if PolarisLaw needs to change that value, Mo'nique can just edit the value of that environment variable and all the tests will use the new value:

```yml
env:
  ALKILN_BEAR_SIZE: "Large bear"
```
...
<!-- TODO: Is "using-workflows" the workflow link we want to use here? Probably, for general workflow information. -->
...
1. ALKiln goes to [its account](security.mdx#alkiln-account) and creates a new Project.
...
1. ALKiln deletes the Project it created from [ALKiln's account](security.mdx#alkiln-account).
1. It makes the [test output](#artifacts), like the report and Scenario folders.


### <AutoDIY/> test stages {#diy-stages}
...
### <KittyLitter/> test stages {#sandbox-stages}
