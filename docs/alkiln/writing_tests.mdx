...
3. You write and edit `.feature` test files in your Sources folder.
...
<!-- TODO: Link to what a Step is. Is that in a separate file? -->
...
[^meh] We are still working finding a more descriptive name than "command". Suggestions are welcome.


## The Story Table Step {#story-table}

The Story Table Step is an [interactive Step](#interact), but it is important enough to have its own section. It is the Step most authors work with the most.


## Interactive Steps {#interactive}

You can use some Steps to change what is on the page by filling in fields or pressing buttons or tabs.
...
## Steps to start tests {#starter}

There are some Steps that test authors tend to use at the very start of a test. They can still be used multiple times and at any point in the test.
...
### `The maximum time for a Step is <number> seconds`
...
- Each file **must** end in `.feature`. For example, `user_has_children.feature`.
...
Here is an example the contents of a file that uses most of these features. The name of this file is `with_children.feature`.
