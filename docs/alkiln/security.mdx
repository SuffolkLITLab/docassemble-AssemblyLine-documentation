...
<!-- TODO:
Components in headings don't show up in the sidebar
-->

<!-- Feedback: Likes detail, but very overwhelmed
...
- Sensitive information
  - Tokens, like AWS tokens
  - User names or passwords
- Code to mess up your server

It's natural to be unsettled by this.
-->
...
<!-- Protect security-critical data  -->
...
<!--
https://sdtimes.com/agile/testing-in-production-risk-vs-reward/
...
- [] Should I trust the authors of kiln
- [] How to use kiln properly
- [] How can I use kiln to give it least permissions and why
- [] How to write tests safely
- [] Why to trust us

- [x] Not sure about purpose - title is confusing. “Security”.

- [] Not tell people about 3rd party packages at all. Part of the problem - these docs don’t explain what third party is. Maybe move to appendix. Maybe say why they should trust is there in appendix.
- [] Everything is 3rd party.

- [] The list items at the very top are not related to each other.

- [x] Disabling is unrelated. Maybe in setup. Maybe mention it in security.

Categories:
- [] Keeping your stuff safe
- [] Keeping your users' stuff safe
...
- [x] [Means mention authors should use a separate server for testing] Mention separate server in GTOYS
...
- [] Databases. Give example, Like google spreadsheets that have real info. Including old databases. (Databases or a copy of real data)

- [] “To actual user info” or old user info.
- [] Maybe link to good external reference describing dangers of using old user info. No suggestions

- [] Principle of least privilege link would be good. No suggestions.

Exposing:
- [] We said don’t run stuff on production, but still don’t want to show test server config keys and passwords. Maybe title to name “passwords, api keys, and similar” (maybe only need first 2) (maybe talk about example of showing config on screen)
- [] “Sensitive info” sounds like user info. Secrets is unclear, though.
...
- [] Maybe worry less to FAQ instead? Or addenda? Small thing
...
In software development, security can be an intimidating topic. By design, ALKiln tests are unable to do very much, but all software comes with risk. No one can eliminate all risks, but we can mitigate them with robust security measures. Over time, we can all build up our security tool chests to:

- Protect our users' information
- Protect our security-critical information
- Protect our servers

One of our strongest tools is the [principle of least privilege](#privilege).

<!-- To start, it is useful to understand what we are protecting against. For this community, some of the concerns about a tool like ALKiln can sound alarming. That is what these documents are for. Some concerns might be, in order of how alarming they sound: -->
...
<!-- Those are real concerns, but there are ways to deal with them. We will discuss ways to protect against these and other challenges. -->
...
- Never has access to server configuration tokens
- Never has access to your current or old production databases
- Can never affect the files on your production server

First and foremost, avoid running tests on your production server[^bonus]. That means <GTOYS/> and <AutoDIY/> tests. Have a separate server for testing. If you are [able to use <KittyLitter plain={true}/> tests](setup.mdx#sandbox-cons), they already make their own server.
...
- Never share real data or databases between servers, whether it is current or old.


[^bonus]: Using a different server for testing is better in other ways, too. When developers pull packages that have python files onto your docassemble server, it reloads for a while. This can cause your clients' interviews to time out. <GTOYS/> tests do the same and can make for a slower user experience.


### Make an account just for ALKiln {#alkiln-account}

**This is for any packages:** that have ever used <GTOYS/> tests
...
As you will see in [ALKiln's test setup interview](https://apps-dev.suffolklitlab.org/start/test-setup) for <GTOYS/> tests, <GTOYS/> tests need access to an account on your docassemble server. If you are using or ever have used <GTOYS/> tests, ALKiln has access to an account.

That account should be just for ALKiln. If you are using an actual user's or developer's account, make a new account. Make sure ALKiln's account only has a [developer role](#alkiln_role) with no extra privileges. That can put a few limits on what the tests have access to. Also, avoid giving other personal or security-critical data to that account, like a real name or GitHub account integration.

We usually call the user "automated_tests" and give the email "automated_tests@example.com" with a secure password.
...
Even if you are using other types of tests, you may want an account for ALKiln. For example, if you want a test to sign into an account, you may want that account to have more limited access. If so, give that account a role that is as limited as possible. If the account just needs to be a user, give it a user role. If the role only needs to be a maintainer, give it a maintainer role.

If you are no longer using the account, delete it.

<!-- TODO: (FAQ) IDs for all blocks are good for caching issues -->


<!-- Docs, docs, the magical task - the more you write, the more they lack -->

bookmark

WIP

<!-- If only using InThePlayground, you don't even need to give it a user account. -->

It makes life easier in other ways too. <!-- TODO: Link -->

<!-- TODO:
Easier because:
- won't make projects on a users account or interviews in their available interviews.
- won't have the developer's API key, so can't do stuff with that
- If the account does something, you can tell that it was the testing account that did that thing
-->

<!-- TODO: add link to GitHub secrets? -->

<!--
TODO: The ALKiln user needs to have a developer account. Add that to setup interview and docs we write here about it.
...
- Have API keys
- Get own id with API
- Create Project on own Playground (for GitHub+You)
- Pull a repo into a Project on own Playground (for GitHub+You) https://docassemble.org/docs/admin.html#playground
- Delete project from Playground (for GitHub+You)
-->


### ALKiln account role {#alkiln_role}

Give the ALKiln docassemble account the [role](https://docassemble.org/docs/users.html) of developer and no [extra permissions](https://docassemble.org/docs/config.html#permissions). That is the most limited role ALKiln can use to do what it needs to do. It still has a lot of access, but that is unavoidable right now. That said, ALKiln is very limited in what it can do.
...
Even after keeping personal and security-critical data away from [the account that runs ALKiln tests](#alkiln-account), there are still ways you can expose some sensitive data in tests. For example, it is possible to deliberately show a server config value on an interview page.

It is possible to use sensitive information to fill in fields in your tests if you do it correctly using environment variables. ALKiln is careful to [protect against showing information you put in environment variables](#alkiln-env-vars).
...
### Always use mock data in tests {#mock data}

**This is for**: Everyone

Determined people can do a lot with very little data. For example, they can [use even anonymized data](https://www.wired.com/2007/12/why-anonymous-data-sometimes-isnt) to identify your users. Avoid giving your ALKiln tests access to real data.
...
You also still have to avoid saving personal or security-critical information in public files and follow other security precautions.
...
<!-- TODO: Get to the point, explain after -->
...
<!-- TODO: Show what a commit sha looks like -->

1. Go to [ALKiln's repository](https://github.com/SuffolkLITLab/ALKiln).
2. Find the id of a commit you need. You need the full commit id which, in our case, is 40 random characters. To do that, go to the main page of [ALKiln's GitHub repo](https://github.com/SuffolkLITLab/ALKiln). Pick the branch of ALKiln that is relevant to you. Near the top, click on “commits". Find the commit that you want. On the right-hand side of that row will be the shortened version of the commit id. To the right of that is the “copy” button. Click that to copy the full commit id.
...
<!-- TODO: Allow disabling action logs and artifacts, or sending them to an email instead.
