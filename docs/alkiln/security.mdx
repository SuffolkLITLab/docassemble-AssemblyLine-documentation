...
- Economics: The ALKiln team hopes this tool will continue to be free and open source software. We hope to never accept venture capital funding. ALKiln uses an MIT license (todo: link). Free does come at a cost, though - we do have less time and fewer resources, so development is steady, but slow.
...
Follow the [principle of least privilege](https://www.cyberark.com/what-is/least-privilege/). That means give software (or people) the least access possible to your server and data that still lets them do what they need to do. For example, if someone just needs to fill out an interview, give them the role of "user", not "developer".

It is one of the most effective ways to protect yourself and your users. Only give ALKiln, or anyone, the access it needs and no more.
...
If your test needs a logged-in user, avoid using a real person's account. Instead, use [ALKiln's account](#alkiln-account) or a completely separate fake account.
...
Even after keeping personal and security-critical data away from [the account that runs ALKiln tests](#alkiln-account), there are still ways you can expose your test server's sensitive data in tests. Even if you are using a testing server, that server still has credentials, like API keys and passwords, and you should avoid exposing those to the ALKiln tests.
...
### How to use sensitive information in <AutoDIY/> tests {#sensitive-info-diy}
...
[Environment variables might contain sensitive information](#sensitive-info-diy). Because of this ALKiln takes extra care with them.
