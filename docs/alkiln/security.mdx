...
Avoid installing <AutoDIY/> tests on your production server. The ALKilnInThePlayground package installs the ALKiln testing framework directly on your server. It is best to avoid installing third-party code on your production server when possible.

You should:

- Avoid running tests on your production server. To avoid this, use the testing server's address in the value of the `SERVER_URL` environment variable. <!-- TODO: Add link to GitHub secrets for `SERVER_URL` -->
- Avoid installing ALKiln on your production server.
...
<!-- TODO: Move this to #env-vars bookmark -->
...
<!-- TODO: Move this to writing_tests.mdx #env-vars bookmark -->
...
<KittyLitter/> are very secure if you use them correctly. You still have to be careful.

You have to avoid using hard-coded urls that are on your server. That is, avoid using `https://my-server.com` and interviews or other pages there. That will send the tests to your server, which goes around all these precautions and makes the tests less secure again.

You also still have to avoid saving sensitive information in public files and follow other security precautions.


## Use sensitive information in <AutoDIY/> tests {#sensitive-info-playground}
...
Sometimes authors do need to pass sensitive information to their tests. For example, some tests require the "user" to log in, so the tests need to know the email and password for that user. You can do this securely in your <AutoDIY/> tests by adding this information to your docassemble server configuration file. ALKiln will use these as environment variables for your tests.
...
When you run the tests in GitHub, you can see the logs of the test workflow job and you can download the results of the test, or "artifacts". That may seem very exposed, but only people who have permissions on the repository, like administrators, collaborators, or moderators, can see or download that information.
