...
<GTOYS/> tests need to be connected to a docassemble account with the [right role](#alkiln_role) (permissions) to work properly. Even if you are using another type of test, though, your tests may need an account. For example, you may want a test to sign into an account before going to an interview.
