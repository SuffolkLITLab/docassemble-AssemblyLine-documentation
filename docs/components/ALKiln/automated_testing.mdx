...
You can run ALKiln in a variety of ways, each with their [pros and cons and other details](alkiln_advanced#multiple-ways-to-run-alkiln). The section below is about setting up different ways of running tests.
...
Use the `tap tab` Step to interact with [ALToolbox tabs](/docs/components/ALToolbox/altoolbox_overview#display-a-series-of-tabs). ALKiln will tap on the tab and then wait until the tab contents are fully visible.
