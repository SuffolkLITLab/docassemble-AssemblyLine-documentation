...
Welcome to the ALKiln testing framework! It helps you check that your docassemble interviews work the way you want them to. ALKiln acts like a user, following your instructions to do things like [fill in fields](alkiln_writing_tests#story-table), [upload files](alkiln_writing_tests#upload), [sign in](alkiln_writing_tests#sign-in), and [check your PDFs for accuracy](alkiln_writing_tests#compare-files).
...
- [Set up ALKiln for your package](alkiln_setup#recommended)
- [Write your first test](alkiln_setup#first-test)
- [Generate a first draft for more complicated tests](alkiln_writing_tests#alkiln_story)

See [a few more reminders here](alkiln_writing_tests#refresh).
...
- [Setting up](alkiln_setup): Learn different ways to install ALKiln for your package or organization and information about those test types
- [Writing tests](alkiln_writing_tests): What you need to know to effectively write tests
- [Troubleshooting](troubleshooting.mdx): Help with common errors and warnings
- Support for [older Versions](alkiln_deprecated): Guides for using older out-of-date features
- [Frequently Asked Questions](alkiln_writing_tests#faq) (FAQ): Find answers to common questions
...
1. You can start using the [simple "it runs" test](alkiln_setup#first-test) right away. It just makes sure that your interview starts without an error. Even that can be useful. For example, if you forget to commit a template to GitHub, that test will catch it.
...
You can write your simple [first test](alkiln_setup#first-test) right away if you want. Wait to write more complex tests till your code is more stable. "More stable" means that the variables for the section you are testing are mostly staying the same.
